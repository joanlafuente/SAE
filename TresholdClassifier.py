...
# Create a folder for the Threshold approach plots, results and prediction
if not os.path.exists(f'{run_path}/Threshold'):
    os.makedirs(f'{run_path}/Threshold')
if not os.path.exists(f'{run_path}/Threshold/Plots'):
    os.makedirs(f'{run_path}/Threshold/Plots')
if not os.path.exists(f'{run_path}/Threshold/Results'):
    os.makedirs(f'{run_path}/Threshold/Results')
...
plt.savefig(f"{run_path}/Threshold/Plots/ROC_Curve_GNN.png")
plt.close()
...
for i, (train_index, test_index) in enumerate(kf.split(training_data, y=training_labels)):
...
plt.savefig(f"{run_path}/Threshold/Plots/boxplot_tresholds.png")
plt.close()
...
plt.ylim(0, 1)
plt.savefig(f"{run_path}/Threshold/Plots/boxplot_metrics.png")
plt.close()
...
np.save(f"{run_path}/Threshold/Predictions/test_preds.npy", test_preds)
...
from utils import *
from models import GCN, Simpler_GCN, Simpler_GCN_Conv, GCN_Att, GCN_Att_Not_res, GCN_Att_Drop_Multihead, GCN_Att_Drop_Multihead_2_Message


# Parameters CONTRASTIVE LEARNING
hidden_channels = 40 # If the model does not have attention mechanism, is not used
out_channels = 30
dropout = 0.1
lr = 0.001
epochs_contr = 500
early_stopping = 40
batch_size_contr = 500
variation = 'GCN_Att_Drop_Multihead_2_Message' # If it is not set on saved files is Simpler_GCN
...
if variation == 'GCN_Att':
    if dropout != 0:
        raise ValueError('Dropout must be 0 for this model')
    model = GCN_Att(dropout=dropout, hidden_channels=hidden_channels, out_channels=out_channels, in_channels=feat_data.shape[1])
elif variation == 'GCN_Att_Not_res':
    if dropout != 0:
        raise ValueError('Dropout must be 0 for this model')
    model = GCN_Att_Not_res(dropout=dropout, hidden_channels=hidden_channels, out_channels=out_channels, in_channels=feat_data.shape[1])
elif variation == 'GCN_Att_Drop_Multihead':
    model = GCN_Att_Drop_Multihead(dropout=dropout, hidden_channels=hidden_channels, out_channels=out_channels, in_channels=feat_data.shape[1])
elif variation == 'GCN_Att_Drop_Multihead_2_Message':
    model = GCN_Att_Drop_Multihead_2_Message(dropout=dropout, hidden_channels=hidden_channels, out_channels=out_channels, in_channels=feat_data.shape[1])
else:
...
if train_contrastive:
    ...
    model = train_node_embedder_supervised(model, graph, optimizer_gcn, 
                                            ...
                                            variation=variation,
                                            name_model=f'./Weights/contr_sup_drop={dropout}_hidd={hidden_channels}_out={out_channels}_lr={lr}_model={variation}_Yelp.pth')
else:
    model.load_state_dict(torch.load(f'./Weights/contr_sup_drop={dropout}_hidd={hidden_channels}_out={out_channels}_lr={lr}_model={variation}_Yelp.pth'))
...
with open(f'./Pickles/embeds_contr_sup_drop={dropout}_hidd={hidden_channels}_out={out_channels}_lr={lr}_model={variation}_Yelp.pkl', 'wb') as file:
    pkl.dump(out, file)

with open(f"./Pickles/train_test_val_masks__contr_sup_drop={dropout}_hidd={hidden_channels}_out={out_channels}_lr={lr}_model={variation}_Yelp.pkl", "wb") as file:
...
if out.shape[1] > 2:
    ...
    plt.legend()
    plt.savefig(f'./Plots/embeds_contr_sup_drop={dropout}_hidd={hidden_channels}_out={out_channels}_lr={lr}_model={variation}_all_Yelp.png')
    plt.close()
    ...
    plt.ylabel('t-SNE feature 2')
    plt.legend()
    plt.savefig(f'./Plots/embeds_contr_sup_drop={dropout}_hidd={hidden_channels}_out={out_channels}_lr={lr}_model={variation}_test_Yelp.png')
    plt.close()
    ...
    plt.ylabel('t-SNE feature 2')
    plt.legend()
    plt.savefig(f'./Plots/embeds_contr_sup_drop={dropout}_hidd={hidden_channels}_out={out_channels}_lr={lr}_model={variation}_train_Yelp.png')
    plt.close()
else:
...
# clf.fit(out[graph.train_mask.cpu().numpy(), :], graph.y[graph.train_mask].cpu().numpy())

# y_pred = clf.predict(out[graph.train_mask.cpu().numpy(), :])

# from sklearn.metrics import classification_report
# report = classification_report(graph.y[graph.test_mask].cpu().numpy(), y_pred, output_dict=True)

# with open(f'./Reports/contr_sup_drop={dropout}_hidd={hidden_channels}_out={out_channels}_lr={lr}_model={variation}_grad_boost_Yelp.txt', 'w') as file:

...
model = GCN_Att_Drop_Multihead(dropout=0.1, in_channels=feat_data.shape[1], hidden_channels=60, out_channels=50)
...
criterion = loss_fn
model, loss_hist, metric_hist, precision_hist = train_node_embedder(model, graph, optimizer_gcn, criterion, percentage=0.1, n_epochs=1000, name_model='best_model_contrastive_dropout_augmentations.pth')
...
plt.ylabel('Value')
plt.legend()
plt.savefig('loss_contrastive_dropout_augmentations.png')
plt.close()
...
plt.ylabel('Value')
plt.legend()
plt.savefig('val_loss_contrastive_dropout_augmentations.png')
plt.close()
...
plt.ylabel('Value')
plt.legend()
plt.savefig('precision_contrastive_dropout_augmentations.png')
plt.close()
...
with open('embeddings_contrastive_dropout_augmentations.pkl', 'wb') as file:
    pkl.dump(out, file)

with open("train_test_val_masks_dropout_augmentations.pkl", "wb") as file:
...
plt.legend()
plt.savefig('embeddings_contrastive_dropout_augmentations.png')
plt.close()
...
# model, loss_hist_cls, f1_hist_cls = train_node_classifier(model, graph, optimizer_gcn, criterion, n_epochs=1000, name_model='best_model_head_dropout_augmentations.pth')
...
# plt.ylabel('Value')
# plt.legend()
# plt.savefig('loss_classifier_dropout_augmentations.png')
# plt.close()
...
# plt.ylabel('Value')
# plt.legend()
# plt.savefig('f1_classifier_dropout_augmentations.png')
# plt.close()
...
# plt.savefig('confusion_matrix_dropout_augmentations.png')
# plt.close()
